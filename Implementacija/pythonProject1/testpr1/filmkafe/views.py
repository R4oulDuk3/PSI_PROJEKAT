...
from .models import Meni, Product, Coupon, Events, Users, Shift, Schedule, Setup, Table, SetupTables, \
    Preference, Preferences, FreeTables, EventReservations, ReservedTables, ReservedTablesNew
...
def apiEventReservations(request):
    ...
    if(res):
        res = res.data
        for each in res:
            event = Events.objects.filter(idevents=each['event']).values_list('start', 'end', 'description', 'picture', 'name')
            event = event[0]
            print(event)
...
def apiCreateUser(request):
    try:
        ...
        print(dat)
        usr = Users.objects.create_user(mail,mail,password, role='User', name=dat['name'], surname = dat['surname'], phone = dat['phone'], salary=0)
...
def apiCreateManager(request):
    usr = Users.objects.create_user('manager@mail.rs', 'manager@mail.rs', 'managerpass.123', role='Manager', name="Gavrilo", surname="Vojteski",phone="+3814464846")
    errcode = usr.save(usr)
...
def apiCreateEvent(request):
    ...
    for each in tabs:
        FreeTables.objects.create(table = Table.objects.get(idtable = each['table']), event = Events.objects.get(idevents =  p['idevents']))
    print(res.errors)
    return Response(res.data)
...
def apiStart(request):
    if(request.user.role=='Waiter'):
        ...
        if(request.user.is_authenticated):
            ...
            sch = Schedule.objects.filter(waiter=request.user.idusers, shift=request.data['idShift'], day__gt=before, day__lt=after)
            print(sch)
            sch = Schedule.objects.filter(waiter = request.user.idusers, shift = request.data['idShift'], day__gt=before, day__lt=after).update(started = datetime.datetime.now())
            print(before)
        return Response("Success")
...
def apiEnd(request):
    if(request.user.role=='Waiter'):
        if(request.user.is_authenticated):
            ...
            before = datetime.datetime.today()-time_change
            after = datetime.datetime.today()+time_change
            sch = Schedule.objects.filter(waiter = request.user.idusers, shift = request.data['idShift'], day__gt=before, day__lt=after).update(ended = datetime.datetime.now())
